...
def validate_base_seq(seq: str,RNAflag: bool=False) -> bool:
    ...
    # define characters of DNA and RNA
...
# generate kmer frequency function
def kmer_frequency(kmer_dict: dict, print_flag: bool=True) -> dict:
    """
    generates kmer frequency dictionary where keys represent frequency of occurrence
    and values represent the number of kmers that occur at that frequency

    Parameters:
    -----------
        kmer_dict: 
            - dictionary where keys represent each kmer and values represent
            number of occurrences in entire file

        print_flag:
            - prints kmer_dict keys and values to console when True
    Returns:
    --------
        dict:
            - dictionary where keys represent frequency of occurrence
            and values represent the number of kmers that occur at that frequency
    """

    # initialize new frequency dictionary (keys = frequency of occurrence, values = number of kmers)
    kmer_freq_dict: dict = {}

    # count frequency of occurrences for kmer_dict
    for v in kmer_dict.values():
        if v in kmer_freq_dict:
            kmer_freq_dict[v] += 1
        else:
            kmer_freq_dict[v] = 1

    # sort by ascending keys
    kmer_freq_dict = dict(sorted(kmer_freq_dict.items(),reverse=False))
    
    # print sorted keys and values to console if desired
    if print_flag:
        print("kmer frequency","number of kmers in this category",sep="\t")
        for k, v in kmer_freq_dict.items():
            print(k,v,sep='\t')

    return kmer_freq_dict


# define kmer count function
def kmer_count(file: str, kmer_size: int, read_length: int) -> dict:
    """
    generates kmer count dictionary where keys represent each kmer and values represent number
    of occurrences in fastq file
    ...
    --------
        dict:
            - dictionary where keys represent each kmer and values represent
            number of occurrences in fastq file
    """

    # initizlize k-mer counting dictionary (kmers are keys, number of occurences are values)
    kmer_dict: dict = {}

    # define total number of kmers
    num_kmers: int = read_length - kmer_size + 1
    ...
    with open(file) as f:
        ...
        while True:
            ...
            # break loop if at end of fastq file
            if line == '':
                break

            # update statement
            if linecount % 1000000 == 0:
                print(int(linecount / 4),'reads processed')

            # grab the correct line (seq line)
            if linecount % 4 == 1:
                
                for i in range(num_kmers):

                    # slice by each kmer_size
                    current_value: str = line[i:i+kmer_size]
                    
                    # if in dictionary, increment, else create new key
                    if current_value in kmer_dict:
                        kmer_dict[current_value] += 1
                    else:
                        kmer_dict[current_value] = 1
        ...
        # print total number of kmers found to the console
        print('you found', sum(kmer_dict.values()),'kmers!\n')

    return kmer_dict


# array of array function from PS9
def array_of_arrays(file: str, read_length: int, num_reads: int, gzipped=True):
    """
    Generate an array of arrays of quality scores, given a file and empty list of lists.
    ...
    # initiate our array
    all_qscores = np.zeros([read_length,num_reads])
            ...
            while True:
                ...
                if linecount % 4 == 3:
                    ...
                    for i, letter in enumerate(newline):
                        ...
                        all_qscores[i,readcount] = phred_score
